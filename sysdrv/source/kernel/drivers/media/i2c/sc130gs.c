	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		__v4l2_ctrl_modify_range(sc130gs->hblank, h_blank,
					 h_blank, 1, h_blank);
		vblank_def = (s64)mode->vts_def - mode->height;
...
static int __sc130gs_power_on(struct sc130gs *sc130gs)
{
	...
	/* wait for sensor to stabilize after reset release, min 20ms */
...
static int sc130gs_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	const struct sc130gs_mode *def_mode = &supported_modes[1];
			...
			const struct i2c_device_id *id)
{
	...
	sc130gs->cur_mode = &supported_modes[1];
